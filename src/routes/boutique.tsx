...
function BoutiquePage() {
  ...
  if (!character) {
    return (
      <LedgerPage maxWidthClass="max-w-4xl">
  ...
  return (
    <LedgerPage maxWidthClass="max-w-4xl">
      <Frame variant="boutique" contentClassName="!flex-col !items-stretch !justify-start text-left [text-shadow:none]">
        <div className="flex items-baseline justify-between mb-3">
          <h1 className={`font-serif text-lg tracking-[0.12em] uppercase ${INK}`}>Boutique</h1>
          <span className={`text-sm font-mono ${PRICE}`}>{Math.round(character.personal_gold)} or personnel</span>
        </div>
        ...
        <div className={`border ${INK_BORDER} px-3 py-3 mb-3`}>
          <div className="flex items-center justify-between mb-1">
            <p className={`text-sm ${INK}`}>Pierre d'âme</p>
            <span className={`text-xs font-mono ${PRICE}`}>{Math.round(soulStoneCost)} or</span>
          </div>
        ...
        <div className={`border ${INK_BORDER} px-3 py-3 mb-4`}>
          <div className="flex items-center justify-between mb-1">
            <p className={`text-sm ${INK}`}>Sceau d'héritage</p>
            {!legacyMaxed && <span className={`text-xs font-mono ${PRICE}`}>{Math.round(legacyCost)} or</span>}
          </div>
          <p className={`text-xs ${INK_MUTED} mb-2`}>
        ...
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 mb-4">
          {PORTRAITS.filter(p => p.premium).map((p) => {
            ...
            return (
              <div key={p.id} className={`border ${INK_BORDER} p-2 text-center`}>
                <div className={`mb-1.5 ${owned ? "" : "opacity-40 grayscale"}`}>
                  <PortraitDisplay portraitId={p.id} size={64} />
                </div>
                <p className={`text-[10px] ${INK_MUTED} mb-1.5 truncate`}>{p.label}</p>
                {owned ? (
                  <button disabled={active || busy === p.id}
                    onClick={() => run(p.id, () => supabase.from("characters" as any).update({ portrait: p.id }).eq("id", character.id))}
                    className={`w-full text-[10px] uppercase border ${active ? "border-emerald-800/50 text-emerald-800" : `${INK_BORDER} ${INK}`} px-1 py-1 hover:bg-black/5 disabled:opacity-60`}>
                  ...
                  <button disabled={busy === p.id || character.personal_gold < PORTRAIT_COST}
                    ...
                    className={`w-full text-[10px] uppercase border border-amber-800/50 ${PRICE} px-1 py-1 hover:bg-amber-800/10 disabled:opacity-30`}>
        ...
        <div className={`border ${INK_BORDER} px-3 py-3 mb-2`}>
        ...
        <button onClick={() => navigate({ to: "/" })} className={`mt-2 text-xs underline ${INK_MUTED} hover:${INK}`}>
          Retour
        </button>
      </Frame>
    </LedgerPage>
